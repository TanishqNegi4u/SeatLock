...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚙️ Starting Minikube (--cpus=4 --memory=6g)..." -ForegroundColor Yellow
...
Write-Host "📦 Building Backend & Frontend Docker images..." -ForegroundColor Yellow
docker build -t seatlock-backend:latest ./seatlock-backend
docker build -t seatlock-frontend:latest ./seatlock-frontend

# Step 4: Apply Kubernetes Manifests
Write-Host "☸️ Applying Kubernetes Manifests (Postgres, 3-Replica Backend, NodePort)..." -ForegroundColor Yellow
kubectl apply -k ./k8s
...
Write-Host "Backend API & Actuator: $backendUrl" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor Cyan
